...
class ResultsManager:
    ...
    def add_student(self, student: Student) -> Student:
        self.dbc.execute("INSERT INTO students (first_name, last_name, date_of_birth, class_code) VALUES (?, ?, ?, ?)", (student.first_name, student.last_name, student.date_of_birth, student.class_code))
        self.conn.commit()
        student.id = self.dbc.lastrowid
        return student

    def add_result(self, result: Result) -> bool:
        self.dbc.execute("SELECT mark FROM results WHERE student_id=? AND course_id=?", (result.student_id, result.course_id))
        previous_mark = self.dbc.fetchone()
        if previous_mark and previous_mark[0] >= result.mark:
            return False
        else:
            self.dbc.execute("INSERT INTO results (student_id, course_id, mark, achieved) VALUES (?, ?, ?, ?)", (result.student_id, result.course_id, result.mark, result.achieved))
            self.conn.commit()
            return True

    def get_results_by_student(self, student_id, only_last=True):
        if only_last:
            self.dbc.execute("SELECT course_id, MAX(mark) FROM results WHERE student_id=? GROUP BY course_id", (student_id,))
        else:
            self.dbc.execute("SELECT * FROM results WHERE student_id=?", (student_id,))
        return self.dbc.fetchall()

    def get_results_by_course(self, course_id, only_last=True):
        if only_last:
            self.dbc.execute("SELECT student_id, MAX(mark) FROM results WHERE course_id=? GROUP BY student_id", (course_id,))
        else:
            self.dbc.execute("SELECT * FROM results WHERE course_id=?", (course_id,))
        return self.dbc.fetchall()
