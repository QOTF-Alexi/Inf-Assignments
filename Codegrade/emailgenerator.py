...
def offerInfo(firstName, lastName, jobTitle):
    while True:
        ...
        annualSalary = Decimal(annualSalary.replace(',', '.'))
        if 20000.00 <= annualSalary <= 80000.00:
            annualSalary = format(annualSalary, ',')
            annualSalary = (str(annualSalary)).replace(',', 'a')
            annualSalary = annualSalary.replace('.', ',')
            annualSalary = annualSalary.replace('a', '.')
            break
        else:
            print("Input error")
    while True:
